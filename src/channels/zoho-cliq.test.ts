  ...
  readEnvFile: vi.fn((keys: string[]) => {
    const store: Record<string, string> = {
      ...
      ZOHO_CLIQ_CHANNEL_ENDPOINT:
        'https://cliq.zoho.com/api/v2/channelsbyname/test-channel/message',
      ZOHO_CLIQ_BOT_UNIQUE_NAME: 'test-bot',
      ZOHO_CLIQ_API_URL: 'https://cliq.zoho.com',
      ZOHO_CLIQ_ACCOUNTS_URL: 'https://accounts.zoho.com',
      ZOHO_CLIQ_CHAT_ID: 'chat-abc',
  ...
  describe('deliver', () => {
    ...
    beforeEach(async () => {
      fetchSpy.mockImplementation(async (input: string | URL | Request) => {
        ...
        if (url.includes('/channelsbyname/') && url.includes('/message')) {
    ...
    it('treats empty 204 send response as success', async () => {
      if (!adapter) return;
    ...
    it('posts via channel endpoint with bot_unique_name', async () => {
      ...
      const calls = fetchSpy.mock.calls.filter((args: unknown[]) => {
        const url = typeof args[0] === 'string' ? args[0] : '';
        return (
          url.includes('/channelsbyname/test-channel/message') &&
          url.includes('bot_unique_name=test-bot')
        );
      });
