...
function createAdapter(): ChannelAdapter | null {
  ...
  /** Zoho channel/bot send endpoints often return 204 or an empty 200 body. */
  async function parseOptionalJsonBody<T>(res: Response): Promise<T> {
    ...
    try {
      ...
      throw new Error(
        `Zoho Cliq returned non-JSON response (${res.status}): ${text.slice(0, 200)}`,
        { cause: err },
      );
    }
  }
  ...
  async function pollMessages(chatId: string, chatName: string, isDm: boolean): Promise<void> {
    try {
      ...
      for (const msg of messages) {
        ...
        log.info('Zoho Cliq inbound message received', {
          chatId,
          messageId: msg.id,
          sender: msg.sender.name,
